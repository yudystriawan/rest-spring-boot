...
import com.example.restspringboot.email.EmailSender;
import com.example.restspringboot.jwt.JWT;
import com.example.restspringboot.jwt.JwtService;
...
public class AuthService {
  ...
  public String register(RegistrationRequest request) {
    ...
    String url = "localhost:8080/api/auth/confirm?token=" + token;
    emailSender.send(request.getEmail(), bodyHtml(request.getName(), url));
  ...
  public String confirmAccount(String token) {
    ...
    if (expiresAt.isBefore(LocalDateTime.now())) {
      ...
      return "Token expired, we will sent your new confirmation email";
    }
  ...
  private void resendConfirmationEmail(User user) {
  ...
  private String bodyHtml(String name, String url) {
    return "<div style=\"font-family:Helvetica,Arial,sans-serif;font-size:16px;margin:0;color:#0b0c0c\">\n"
               ...
               + " 0;font-size:19px;line-height:25px;color:#0b0c0c\">Hi "
        + name
        + ",</p><p style=\"Margin:0 0 20px 0;font-size:19px;line-height:25px;color:#0b0c0c\">"
        + " Thank you for registering. Please click on the below link to activate your account:"
        + " </p><blockquote style=\"Margin:0 0 20px 0;border-left:10px solid #b1b4b6;padding:15px"
        + " 0 0.1px 15px;font-size:19px;line-height:25px\"><p style=\"Margin:0 0 20px"
        + " 0;font-size:19px;line-height:25px;color:#0b0c0c\"> <a href=\""
        + url
